...
std::unordered_map<std::string, std::function<MatrixXd(const MatrixXd&, bool)>> Layer::activationMap = {
    {"relu", relu},
    {"sigmoid", sigmoid},
    {"softmax", softmax}
};

Layer::Layer(unsigned int input_shape, unsigned int size, const std::string &activation_) : size(size), input_shape(input_shape)
{
